...
class UserApplicationService(
    ...
    fun getUserByUserId(userId: UUID): UserDto? {
        log.debug("Trying to get user with id {}", userId)
        val user = userRepository.findByUserId(UserId(userId))
        log.info(user?.let { "Found user $it" } ?: "User with id $userId could not be found")
        return user?.let { UserDto(it) }
    }
    ...
    fun updateUser(
        ...
        log.debug("User {} is trying to update user with command: {}", authenticatedUserUsername, command)
